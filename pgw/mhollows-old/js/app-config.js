...
function openAnnouncement(announcement, type, state, ignoreDeviceState) {
  dbg("OPEN ANNOUNCEMENT", announcement, type, state);

  if (type == 'event' || type == 'event_recap') {
    if (!ignoreDeviceState) {
      dbg('1 REDIRECT TO ' + '/event-detail/' + announcement.get('event').id);
      state.go('app.event-detail', {eventId: announcement.get('event').id});
    }
    else {
      setTimeout(function() {
        dbg('2 REDIRECT TO ' + '/event-detail/' + announcement.get('event').id);
        state.go('app.event-detail', {eventId: announcement.get('event').id});
      }, 2000);
    }
  } else {
    ...
    if (announcement.get('type') == 'url') {
      var ref = window.open(announcement.get('url'), '_blank', 'location=no,EnableViewPortScale=yes');
    } else {

      if (!ignoreDeviceState) {
        dbg('1 REDIRECT TO ' + '/announcement/' + announcement.id);
        state.go('app.announcement', {id: announcement.id});
      }
      else {
        setTimeout(function() {
          dbg('2 REDIRECT TO ' + '/announcement/' + announcement.id);
          state.go('app.announcement', {id: announcement.id});
        }, 2000);
      }
    }
...
window.initializeParsePlugin = function () {
  ...
  //   dbg('yo i got this push notification:' + JSON.stringify(pn));
  //   window.pushNotifications.onMessageReceived(pn);
  // });
}
  ...
  navigator.notification.alert(
          ...
          function () {
            Parse.Config.get().then(function (config) {
              ...
              var ref = window.open(downloadURL, '_self', 'location=no');
            });
          },
  ...
  navigator.notification.confirm(
          ...
          function (buttonIndex) {
            if (buttonIndex == 2) {
              Parse.Config.get().then(function (config) {
                var downloadURL = 'http://' + config.get('groupfireId') + '.admin.groupfire.com/download';
                var ref = window.open(downloadURL, '_blank', 'location=no');
              });
            }
          },
          'New Version Available!',
              ...
              if (has) {
                ...
                webintent.getExtra("com.parse.Data",
                  function (d) {
                    ...
                    window.pushNotifications.onMessageReceived(metadata, true);
...
document.addEventListener('resume', function () {
  ...
  if (currentUser) {
    ...
    if ($('.page-feed').length > 0) {
      angular.element($('.page-feed')[0]).scope().loadData();
    }
    //reload content pages
    if ($('.list.content-pages').length > 0) {
      var AppCtrl = angular.element($('.list.content-pages')[0]).scope();
