        ...
        .controller('EventDetailController', function ($scope, EventUtils, Utils, $location, $stateParams, $state, $timeout, $ionicPopover) {
          ...
          $scope.loadEvent = function (eventId) {
            ...
            query.get(eventId).then(function (event) {
              Utils.trackMixPanel("View Event", {
                "Event ID": event.id,
          ...
          $scope.viewMemberDetail = function (m, attendees, index) {
            if (attendees[index].get('user').get('profile'))
              $location.url('/profile/' + attendees[index].get('user').get('profile').id);
            else
              Utils.alert('Profile you selected doesn\'t exist. ');
          };
          ...
          $scope.addToCalendar = function () {
            ...
            var confirmTemp = '<p style=" font-size: 13px; margin:0px;">You are adding the following event to your calendar.</p>\n\
                                <p style="margin:14px 0px 0px 0px;font-weight:bold;">'+$scope.event.get('title')+' \n\
                                <p style="margin:3px 0px 0px 0px;font-size: 12px;"><i class="svg-icon clock-black" \n\
                                  style="height:15px;width:20px;font-size:14px; margin-top: -2px;"></i> '+EventUtils.displayEventDate($scope.event.get('startAt'))+'</p>\n\
                                <p style="margin:3px 0px 0px 0px;font-size: 12px;"><i class="svg-icon pin-black" \n\
                                  style="height:15px;width:20px;font-size:14px; margin-top: -2px;"></i> '+$scope.event.get('streetAddress1')+'</p>';
            $scope.hideAddToCalendarConfirm();
            Utils.confirm(confirmTemp, 'Add to Calendar', function () {
              if (window.plugins && window.plugins.calendar) {
                ...
                window.plugins.calendar.createEvent(
                        ...
                        function () {
                          Utils.hideIndicator();
                          Utils.confirm('Event name has been added to your calendar. Go to calendar?', 'Add to Calendar', function () {
                            window.plugins.calendar.openCalendar(startDate.toDate()); // callbacks are optional
                          });
                          $scope.hideAddToCalendarConfirm();
                        },
                        function (msg) {
          ...
          $scope.registerEvent = function () {
            ...
            Utils.trackMixPanel("Register for Event", {
          ...
          $scope.cancelRegistration = function () {
            Utils.trackMixPanel("Cancel Event Registration", {
              "Event ID": $scope.event.id,
              "Event Title": $scope.event.get('title')
            });
