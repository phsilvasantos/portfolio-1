...
angular.module('gnApp.controllers')
        .controller('GroupFeedMoreController', function ($scope, $stateParams, Utils, $ionicPopover, $state, $ionicSideMenuDelegate) {
            ...
            query.find().then(function (groupMembers) {
              ...
              $scope.$apply(function () {
                ...
                if (groupMembers.length > 0) {
                  ...
                  $scope.onSelectNotificationsPeriod($scope.periods[0], true);
                }
                var filtered = $scope.periods.filter(function (item) {
                  return item.mins == periodMins;
                });
                if (filtered.length === 0) {
                  $scope.onSelectNotificationsPeriod({mins: periodMins, label: periodMins + ' minutes'}, true);
                } else {
                  $scope.onSelectNotificationsPeriod(filtered[0], true);
                }
              });
            });
          };
          ...
          $scope.muteToggle = function(){
            $scope.notificationSettings.mute = !$scope.notificationSettings.mute;
          ...
          $scope.onSelectNotificationsPeriod = function (item, ignoreTriggerSave) {
