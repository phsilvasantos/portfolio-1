...
xmppGroupChat = (function() {
  ...
  xmppGroupChat.registerXmppHandlers = function() {
    ...
    if (!xmppConnectionData) {
      xmppConnectionData = {};
      xmppConnectionData.sentMessages      = [];
    ...
    xmppConnection.unregisterMessage = function(uniqueId) {
      var index = xmppConnectionData.processedMessages.indexOf(uniqueId);

      if (index > -1) {
        xmppConnectionData.processedMessages.splice(index, 1);
    ...
    xmppConnection.sendMessage = function(messageBuilder, msgid) {
      ...
      xmppConnectionData.sentMessages.push(msgid);
    ...
    xmppConnection.muc.dropUserFromConversation = function(room, userId) {
      var msg;
      var msgid = xmppGroupChat._generateMessageUid();
    ...
    xmppConnection.muc.destroyRoom = function(room) {
      var msg;
      var msgid = xmppGroupChat._generateMessageUid();
  ...
  xmppGroupChat.uiHandlers = {
    ...
    onRoomDestroyed: function(room) {
    },
    onUserWasDroped: function(room, userId) {
    }
  };
  ...
  xmppGroupChat.xmppHandlers = {
    onMessageReceived: function(message) {
      ...
      debug(function() {
        ...
        xmppGroupChat.choseMessageHandler('onMessageDelivered', message, function() {
          var isMessageWasSentFromThisDevice = xmppConnectionData.sentMessages.indexOf(Strophe.getText(message.getElementsByTagName('uniqueid')[0]) * 1) > -1;
          var isMessageHasContent            = message.getElementsByTagName('body').length > 0;
          var isMessageSentByCurrentUser     = xmppGroupChat._userById(message.getAttribute('from')) == xmppAuth.currentUser.id.toLowerCase()
          var isMessageNotFromArchive        = message.getElementsByTagName('delay').length == 0;

          return isMessageWasSentFromThisDevice && isMessageHasContent && isMessageSentByCurrentUser && isMessageNotFromArchive;
        });
        xmppGroupChat.choseMessageHandler('onTopicChange', message, function() { return message.getElementsByTagName('subject').length > 0 });
        xmppGroupChat.choseMessageHandler('onUserWasDroped', message, function() { return message.getElementsByTagName('drop_user').length > 0 });
        xmppGroupChat.choseMessageHandler('onRoomDestroyed', message, function() { return message.getElementsByTagName('destroy').length > 0 });
    ...
    onUserWasDroped: function(message) {
      var elems = message.getElementsByTagName('drop_user');

      var roomName = xmppGroupChat._roomNamebyID(message.getAttribute('from').split('/')[0]);
      var userName = xmppGroupChat._userById(message.getAttribute('from'));
      var userId   = Strophe.getText(elems[0]);

      xmppGroupChat.uiHandlers.onUserWasDroped(roomName, userId);
    },

    onRoomDestroyed: function(message) {
      var roomName = xmppGroupChat._roomNamebyID(message.getAttribute('from').split('/')[0]);

      xmppGroupChat.uiHandlers.onRoomDestroyed(roomName);
    },
  ...
  xmppGroupChat.leave = function(roomName) {
      ...
      setTimeout(function() {
        xmppGroupChat.leaveRemoteRoom(roomName);
        xmppGroupChat.xmppHandlers.onRoomLeaved(roomName);
      }, 2000);
  };

  xmppGroupChat.leaveRemoteRoom = function(roomName) {
  ...
  xmppGroupChat.destroyRoom = function(roomName) {
    xmppConnection.muc.destroyRoom(xmppGroupChat._roomIDbyName(roomName));
  };

  xmppGroupChat.dropUserFromConversation = function(userId, roomName) {
    xmppConnection.muc.dropUserFromConversation(xmppGroupChat._roomIDbyName(roomName), userId);
  }
