angular.module('myApp.directives', [])
        ...
        .directive('mspDecimal', ['$filter', function ($filter) {
            return {
              require: '?ngModel',
              restrict: 'A',
              link: function (scope, elem, attr, ctrl) {
                if (ctrl) {
                  ctrl.$formatters.push(function (viewValue) {
                    return $filter('mspNumber')(viewValue);
                  });
                }
                elem.bind('keydown', function (event) {
                  if ((event.keyCode > 47 && event.keyCode < 58) || (event.keyCode > 95 && event.keyCode < 106) || [190, 8, 9, 46, 37, 39, 110].indexOf(event.keyCode) !== -1) {
                  } else {
                ...
                elem.bind('blur', function () {
                  if ($(this).val()) {
                    $(this).val($filter('mspNumber')($(this).val()));
                  }
                });
              }
            };
