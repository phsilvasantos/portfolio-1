...
function moveOnTouch ($el, options) {
  ...
  function onEnd (result) {
    ...
    var cooDiff = backCoo - coo,
        ...
        //swipeFLAG = !longTouchFLAG && Math.abs(moveElPos-startElPos) > 30 && newPos === moveElPos;
	    swipeFLAG = !longTouchFLAG && moveElPos !== startElPos && newPos === moveElPos;
		// [customized] end
...
jQuery.Fotorama = function ($fotorama, opts) {
  ...
  function setNavShaftMinmax () {
	...
	*/
	  var padding = $nav.customnavpaddingleft;
	  if (padding == null) {
		  padding = 0;
	  }
	  navShaftData.min = Math.min(0, measures.W - padding - $navShaft.width());
	// [customized] end
    navShaftData.max = 0;
    ...
    if (measureIsValid(width)) {
      ...
      if (height) {
        ...
        if (o_nav) {
		  ...
		  */
			var paddingNav = 0;
			if (opts.customnavpaddingleft) {
				paddingNav = opts.customnavpaddingleft;
			}
			$nav.customnavpaddingleft = paddingNav;
			$nav.parent().css("padding-left", paddingNav);
			$nav
				.stop()
				.animate({width: width - paddingNav}, time);
		  // [customized] end
